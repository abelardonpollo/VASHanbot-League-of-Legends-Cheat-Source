...
ULONG LdrHashEntry(UNICODE_STRING UniName, BOOL XorHash) {
	ULONG ulRes = 0;
	char procName[] = "RtlHashUnicodeString";
	RTLHASHUNICODESTRING pRtlHashUnicodeString = (RTLHASHUNICODESTRING)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), procName);
...
PRTL_RB_TREE FindModuleBaseAddressIndex()
{
	...
	*/
	char procNamse[] = "RtlCompareMemory";
	char procNams[] = "strcmp";
	RTLCOMPAREMEMORY pRtlCompareMemory = (RTLCOMPAREMEMORY)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), procNamse);
	STRCMP pstrcmp = (STRCMP)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), procNams);

	wchar_t dllName[] = L"ntdll.dll";
	PLDR_DATA_TABLE_ENTRY2 pLdrEntry = FindLdrTableEntry(dllName);
...
{
	char procNamse[] = "RtlRbInsertNodeEx";
	RTLRBINSERTNODEEX pRtlRbInsertNodeEx = (RTLRBINSERTNODEEX)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), procNamse);
...
{
	char procNamse[] = "MultiByteToWideChar";
	MULTIBYTETOWIDECHAR pMultiByteToWideChar = (MULTIBYTETOWIDECHAR)GetFunctionAddress(IsModulePresent(L"kernel32.dll"), procNamse);
...
)
{
	...
	HMODULE kernel32_addr = IsModulePresent(L"KERNEL32.dll");
	...
	HMODULE kernelbase_addr = IsModulePresent(L"KERNELBASE.dll");
	...
	*FunctionAddress = NULL;
	...
	STRING funcname_s = { 0 };
	const char* funcname_s = "LdrGetProcedureAddress";
	FILL_STRING(funcname_s, funcname_s);
...
)
{
	...
	for (int i = 0; i < pNtHeaders->FileHeader.NumberOfSections; i++)
	{
		...
		if (my_strncmp(".text", (char*)pSecHeader->Name, 6))
		{
	...
	if (pDataDir->Size)
	{
		...
		for (int i = 0; i < numberOfEntries; i++)
		{
			...
			if (ProcName != NULL)
			{
				...
				if (my_strlen((char*)functionName) != ProcName->Length)
					continue;
				if (my_strncmp((char*)functionName, ProcName->Buffer, ProcName->Length))
				{
					// found it
					found = TRUE;
					short* pRVA2 = RVA(
			...
			if (found)
			{
				...
				if (startValidSection > FunctionPtr || FunctionPtr > endValidSection)
				{
					...
					if (lib_length != 0)
					{
						...
						HMODULE lib_addr = IsModulePresentA(libname);
				...
				/*if (ProcName && my_strncmp("CreateThread", ProcName->Buffer, ProcName->Length))
				{
					DedbgA("Ìæ»»CreateThread");
					DedbgA("Ìæ»»CreateThread");
					DedbgA("Ìæ»»CreateThread");
					DedbgA("Ìæ»»CreateThread");
					DedbgA("Ìæ»»CreateThread");
					...
					DedbgA("Ìæ»»CreateThread");
					DedbgA("Ìæ»»CreateThread");
					MessageBoxA(0, "Ìæ»»CreateThread", 0, 0);
					FunctionPtr = HookCreateThread;
				}
				if (ProcName && my_strncmp("VirtualAlloc", ProcName->Buffer, ProcName->Length))
				{
					DedbgA("Ìæ»»VirtualAlloc");
					DedbgA("Ìæ»»VirtualAlloc");
					DedbgA("Ìæ»»VirtualAlloc");
					DedbgA("Ìæ»»VirtualAlloc");
					DedbgA("Ìæ»»VirtualAlloc");
					...
					DedbgA("Ìæ»»VirtualAlloc");
					DedbgA("Ìæ»»VirtualAlloc");
					MessageBoxA(0, "Ìæ»»VirtualAlloc", 0, 0);
					FunctionPtr = HookVirtualAlloc;
				}*/
...
)
{
	...
	char procNamse[] = "GetProcessHeap";
	char procNamse2[] = "HeapAlloc";
	char procNamse3[] = "RtlInitUnicodeString";
	char procNamse4[] = "NtQuerySystemTime";
	GETPROCESSHEAP pGetProcessHeap = (GETPROCESSHEAP)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), procNamse);
	HEAPALLOC pHeapAlloc = (HEAPALLOC)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), procNamse2);
	RTLINITUNICODESTRING pRtlInitUnicodeString = (RTLINITUNICODESTRING)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), procNamse3);
	NTQUERYSYSTEMTIME pNtQuerySystemTime = (NTQUERYSYSTEMTIME)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), procNamse4);
	...
	//auto b = (PLDR_INIT_ROUTINE)RVA(
	//	PVOID,
	//	pdModule->ModuleBase,
	//	pNtHeaders->OptionalHeader.AddressOfEntryPoint
	//);

	//DedbgA("pLdrEntry->EntryPoint:%p|%p", b, &pLdrEntry->EntryPoint);
	//DedbgA("pLdrEntry->EntryPoint:%p|%p", b, &pLdrEntry->EntryPoint);
	//DedbgA("pLdrEntry->EntryPoint:%p|%p", b, &pLdrEntry->EntryPoint);
	//DedbgA("pLdrEntry->EntryPoint:%p|%p", b, &pLdrEntry->EntryPoint);
	//DedbgA("pLdrEntry->EntryPoint:%p|%p", b, &pLdrEntry->EntryPoint);
	...
	//MessageBoxA(0, 0, 0, 0);
	// set the entry point
	pLdrEntry->EntryPoint = (PLDR_INIT_ROUTINE)RVA(
		PVOID,
		pdModule->ModuleBase,
		pNtHeaders->OptionalHeader.AddressOfEntryPoint
	);
