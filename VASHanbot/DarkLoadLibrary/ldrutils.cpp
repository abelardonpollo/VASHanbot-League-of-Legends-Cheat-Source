...
)
{
    ...
    //    NTSTATUS status = syscall::call(syscall::index("NtAllocateVirtualMemory"), 
    //    (HANDLE)-1,
    //    (PVOID*)&pdModule->ModuleBase,
    //    0,
    //    &RegionSize,
    ...
    //if (!NT_SUCCESS(status) || pdModule->ModuleBase != pNtHeaders->OptionalHeader.ImageBase)
    {
        ...
        RegionSize = pNtHeaders->OptionalHeader.SizeOfImage;
    	status = syscall::call(syscall::index("NtAllocateVirtualMemory"),
            (HANDLE)-1,
            (PVOID*)&pdModule->ModuleBase,
            0,
            &RegionSize,
    ...
    Ace_Patch::g_bot_info.base_address = pdModule->ModuleBase;
    Ace_Patch::g_bot_info.module_size = RegionSize;
    Ace_Patch::AddModuleInfo({ Ace_Patch::g_bot_info.base_address , Ace_Patch::g_bot_info.module_size });
...
)
{
    ...
    PRUNTIME_FUNCTION pFuncEntry;
         ...
         if (pDataDir->Size)
         {
             ...
             RtlAddFunctionTable(
                 (PRUNTIME_FUNCTION)pFuncEntry,
                 (pDataDir->Size / sizeof(IMAGE_RUNTIME_FUNCTION_ENTRY)) ,
                 pdModule->ModuleBase
             );
         }
     #endif
