...
SIZE_T WideStringLength(LPCWSTR str)
{
	SIZE_T len = 0;
	SIZE_T i = 0;
...
)
{
	...
	{
		pdModule->ErrorMsg = _wcsdup(L"Failed to allocate memory");

		return FALSE;
	}

	auto lpwFileNameLocation = pPathFindFileNameW(lpwFileName);
...
{
	char procNamse[] = "HeapAlloc";
	char procNamse1[] = "CreateFileW";
	char procNamse2[] = "VirtualAlloc";
	char procNamse3[] = "GetFileSize";
	char procNamse4[] = "ReadFile";
	char procNamse5[] = "CloseHandle";
	HEAPALLOC pHeapAlloc = (HEAPALLOC)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), procNamse);
	CREATEFILEW pCreateFileW = (CREATEFILEW)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), procNamse1);
	VIRTUALALLOC pVirtualAlloc = (VIRTUALALLOC)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), procNamse2);
	GETFILESIZE pGetFileSize = (GETFILESIZE)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), procNamse3);
	READFILE pReadFile = (READFILE)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), procNamse4);
	CLOSEHANDLE pCloseHandle = (CLOSEHANDLE)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), procNamse5);
	...
	pdModule->pbDllData = (PBYTE)pVirtualAlloc(
        NULL, 
        dwSize, 
...
)
{
	...
	*/
	...
	case LOAD_MEMORY:
		...
		dModule->pbDllData = (PBYTE)lpFileBuffer;
